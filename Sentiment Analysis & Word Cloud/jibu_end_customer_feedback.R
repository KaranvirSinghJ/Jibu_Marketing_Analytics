...
#Import dataset
jiburesponses <- read_excel("jiburesponses.xlsx", sheet = "Sheet2")
View(jiburesponses)

#Reading column names
headers<-colnames(jiburesponses)
headers
...
summary(jiburesponses$feedback)


# Text Preprocessing
additional_stopwords <- c("in", "is", "jibu", "'ve", "'s","the","bottles","water")

corpus <- Corpus(VectorSource(jiburesponses$feedback))
corpus_clean <- corpus %>%
